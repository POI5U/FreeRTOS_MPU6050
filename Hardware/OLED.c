...
/**
  * @brief  OLED显示一个浮点数(默认Signed)
  * @param  Line 行位置，范围：1~4
  * @param  Column 列位置，范围：1~16
  * @param  Number 要显示的浮点数
  * @param  Length 要显示浮点数的长度（同时代表小数点前后）
  * @retval 无
  */
...
{	
	...
	OLED_ShowNum(Line, Column+1, (int)Number1, Length);
	OLED_ShowChar(Line, Column+1+Length, '.');
	OLED_ShowNum(Line, Column+2+Length, (Number1 - (int)Number1) * OLED_Pow(10, Length) , Length);
